...
clienteEndpoints.post('/cliente', async (req, resp) =>{
    try{
        ...
        if(cliente.cpf.length < 14)
            throw new Error('A quantidade de números no CPF está incorreto.')
        if(cliente.telefone.length < 15)
            throw new Error('A quantidade de números no telefone está incorreto.')
        if(!cliente.email)
...
clienteEndpoints.post('/endereco/:num', async (req, resp) => {
    try{
        ...
        if(!endereco.idCliente || endereco.idCliente === 0)
            throw new Error('Id cliente inválido ou não encontrado')
